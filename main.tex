...
\begin{definition}
    ...
    An arrow $g : X \to Z$ \textbf{lifts} $\vec p$-colimits when if $(d, \lambda)$ is a colimiting $\vec p$-cocone for $gf$, there exists a limiting $\vec p$-cocone $(c, \alpha)$ for $f$ such that $(gf, g\alpha)$ is isomorphic to
\end{definition}
\todo{Fix the definition}

\begin{definition}
    \index{limits!creation}\index{colimits!creation}
    An arrow $g : X \to Z$ \textbf{creates} (non-strictly) $\vec p$-colimits if it both lifts and reflects $\vec p$-colimits. Dually, an arrow $g : X \to Z$ \textbf{creates} (non-strictly) $\vec p$-limits if it both lifts and reflects $\vec p$-limits.
\end{definition}
...
\begin{definition}
    ...
    Let $j : A \to E$ be an arrow and $\ell\dashv_j r, \ell'\dashv_jr'$ $j$-relative adjunctions. A \textbf{left morphism} from $\ell\dashv_jr$ to $\ell'\dashv_j r'$ consists of
    \[\begin{tikzcd}
...
\begin{example}
    For any arrow $j : A \to E$ and $j$-relative adjunction $\ell\dashv_j r$, $(r, \eta)$ forms a left morphism from $\ell\dashv_j r$ to $j\dashv_j \id_E$. The latter is then terminal in $\cat{RAdj}_L(j)$.
\end{example}
...
\section{Algebras and opalgebras}

In the classical settings, two categories of algebras are usually studied: the Eilenberg-Moore category ("all the algebras") and the Kleisli category ("free algebras"). Internal notions in virtual equipment exists: algebras and opalgebras. While the first ones are known to the reader, the latter are less so. Although in the classical setting free algebras and opalgebras coincide, in the formal setting the latter notion is of importance.
...
\begin{definition}
    ...
    \hspace{1em}=\hspace{1em}
    \begin{tikzcd}[sep=large]
        \ar[d, equal] A & \ar[l, loose, "{E(j, t)}"'] A \ar[d, equal] & \ar[l, loose, "{E(j, e)}"', ""{anchor=center, name=t}] D \ar[d, equal]\\
        \ar[d, equal] A & \ar[l, loose, "{E(j, t)}"'] A & \ar[l, loose, "{E(t, e)}"', ""{anchor=center, name=m}] D\ar[d, equal]\\
        \ar[d, equal] A &{}& \ar[ll, loose, "{E(j, e)}"] D \ar[d, equal]\\
        A &{}& \ar[ll, loose, "{E(t, e)}"] D
        \ar[from=t, to=m, phantom, "\rtimes"{anchor=center}]
        \ar[from=2-2, to=3-2, phantom, "{\smallsmile_t(j, e)}"{anchor=center}]
        \ar[from=3-2, to=4-2, phantom, "\rtimes"{anchor=center}]
    \end{tikzcd}\]
    A $T$-algebra morphism from $(e, \rtimes)$ to $(e', \rtimes')$ is a cell $\varepsilon : e \To e'$ such that
    \[\begin{tikzcd}
