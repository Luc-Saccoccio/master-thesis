...
Justin Carel, Hadrien Chalandon-Goskrzynski, Ugo Fialo, Rafael Escabias (tbd, proof-reading)
...
The universal property gives an identity or a composite (eventually left- or right-biased) and associativity (idem) up to an invertible cell; when the formers exists. In the case of the composite, asking for equality is too strict. In the case of identity however, there's often a 'canonical choice'.
...
\begin{theorem}[{\cite[Theorem 7.16]{cs10}}]\label{th:comp_rest}
    Let $p : B \dfrom C$, $f : A \to B$ and $g : D \to C$. Then $B(f, 1) \odot p \odot C(1, g)$ is isomorphic to $p(f, g)$ when one of the sides exists.
\end{theorem}
...
\begin{proof}
    %TODO
\end{proof}
\todo{commencer, finir et expliquer}
...
\end{proof}
\todo{exact ou cocontinue ? typo dans \cite{arkor2026presheavescocompletionsformalcategory} ?}
...
\todo{focus sur les préfaisceaux -> loc internes, faciles à construire}
...
\begin{definition}[{\cite[Definition 7.4.1]{riehl_verity}}]
    ...
    A \textbf{module} from $A$ to $B$ in a $\infty$-cosmos is a two sided fibration $A \from E \to B$ that is a discrete object in $\prescript{}{A\textbackslash}\Fib(\ca K)_{/B}$ (i.e. the functor spaces with codomain the module are Kan complexes).
\end{definition}
...
Lastly, we need morphism of modules:
...
Recall the many $\infty$-cosmoï of models of $(\infty, 1)$-categories introduced in Example \ref{ex:infcats_cosmoi}, related using multiple cosmological biequivalences as mentioned in Example.





\todo{problème des homotopy coherent monads}
\todo{mentionner les \texorpdfstring{$\infty$}{∞}-équipements de \cite{ruit2025formalcategorytheoryinftyequipments} comme potentielle solution ?}
