...
\begin{notation}
    ...
    \[\begin{tikzcd}
        ...
        \ar[from=t, to=b, phantom, "\opcart"{anchor=center}]
    \end{tikzcd}
    \hspace{1em}=\hspace{1em}
    \begin{tikzcd}
...
\begin{definition}
    ...
    \[\begin{tikzcd}[sep=huge]
        ...
        \ar[from=2-3, to=2-2, phantom, ""{name=1}] \ar[from=1, to=2, phantom, "{\smallsmile_{\colim^{\vec{p}}f}(f, 1)}"{anchor=center}]
    \end{tikzcd}\]
    ...
    \[\begin{tikzcd}[sep=huge]
        ...
        \ar[from=2-3, to=2-2, phantom, ""{name=1}] \ar[from=1, to=2, phantom, "{\smallsmile_{\lim^{\vec{p}}f}(1, f)}"{anchor=center}]
    \end{tikzcd}\]
\end{definition}
...
In particular, the left Kan extension of $f$ through $j$ is the $X(j, 1)$-colimit of $f$. We can then deduce the following properties:
...
\todo{Ajouter les defs et les 20 000 lemmes qui vont avec}
\todo{add lemmas from \cite[Appendix]{diliberticodensity}}
...
\todo{Ajouter TOUT, en particulier les monades relatives et les résultats sur leurs algèbres !}
...
\begin{corollary}
    ...
    \begin{enumerate}[ref=\ref{cor:psh} (\alph*)]
        \item \label{cor:psh:dense_iff_ff} Let $p : A \dfrom X$ in $\PP$. $\widearc{p} : X \to \psh\PP A$ is fully faithful if and only if $p$ is dense.
...
\begin{proof}
    ...
    And the full faithfulness of restriction gives the commutativity up to isomorphism.
\end{proof}
...
\begin{lemma}[coYoneda Lemma]
    There's an isomorphism $\copsh\PP A(1, \oy_A^\PP) \simeq \pi_A$.
\end{lemma}
...
\todo{Ajouter les résultats sur les cocompletions}
